    ...
    const load = async () => {
      try {
        const response = await fetch(
          `/api/live-streams/public?club=${encodeURIComponent(clubSeleccionado.name)}`,
          { cache: "no-store" }
        );
        const data = await response.json();
        if (!active) return;
        const next = Object.fromEntries(
          ((data.streams || []) as CourtStream[]).map((stream) => [
            stream.court,
            `https://www.youtube.com/watch?v=${stream.videoId}`,
          ])
        );
        setCourtStreams(next);
      } catch {
        if (active) setCourtStreams({});
      }
    };
            ...
            {partidosMostrados.map((partido) => (
              ...
              >
                ...
                <div className="border-t border-zinc-100 px-3 py-3">
                  <StreamButton
                    streamUrl={courtStreams[partido.cancha] || partido.stream_url}
                  />
                </div>
              </article>
            ))}
